...
//	incident.x /= 2.*PI;
	...
	phiIntensity = ((1.-applyLog)*pow(phiIntensity,cubeRoot)+applyLog*log(phiIntensity+1.))*100.;
	
	float thetaInstensity = length(displayChannel*positionThetaIncIntensity);
	thetaInstensity = ((1.-applyLog)*pow(thetaInstensity,cubeRoot)+applyLog*log(thetaInstensity+1.))*100.;
	
	// compute vertex position and auxiliary points in world space.
	vec3 v0 = vec3(vertex.xyz * max(0.,mainIntensity));
	...
	gl_Position = gl_ModelViewProjectionMatrix*vec4(vec3(vertex.xyz * max(0.,mainIntensity)),1.);

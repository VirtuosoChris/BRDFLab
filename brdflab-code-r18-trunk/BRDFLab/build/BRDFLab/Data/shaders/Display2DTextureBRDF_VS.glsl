...
//		positionThetaInc.y -= 1.;
	...
	float mainIntensity = transformReflectionValue(intensity);
	float phiIntensity = transformReflectionValue(positionPhiIncIntensity);
	float thetaIntensity = transformReflectionValue(positionThetaIncIntensity);
	
	// compute vertex position and auxiliary points in world space.
	vec3 v0 = in_vertex.xyz * max(mainIntensity,0.);

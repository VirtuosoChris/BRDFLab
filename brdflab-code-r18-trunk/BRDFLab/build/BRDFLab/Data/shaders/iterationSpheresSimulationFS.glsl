...
void main()																					
{
	...
	if(iDir.a != 1.){
		vec3 iPoint = texture2D(hits,vec2(uv.x,1.-uv.y)).xyz;
		...
		if(iDir.x > 0.){
			tMaxX = length(rayPlaneIntersection(origGridSpace, iDir.xyz, float(X)+1., vec3(-1.,0.,0.))-origGridSpace);
			tDeltaX = length(rayPlaneIntersection(vec3(0.,0.,0.), iDir.xyz, 1., vec3(-1.,0.,0.)));
			stepX = 1;
		}else{
			tMaxX = length(rayPlaneIntersection(origGridSpace, iDir.xyz, -float(X), vec3(1.,0.,0.))-origGridSpace);
			tDeltaX = length(rayPlaneIntersection(vec3(0.,0.,0.), iDir.xyz, -1., vec3(1.,0.,0.)));
			stepX = -1;
		}
		
		if(iDir.y > 0.){
			tMaxY = length(rayPlaneIntersection(origGridSpace, iDir.xyz, float(Y)+1., vec3(0.,-1.,0.))-origGridSpace);
			tDeltaY = length(rayPlaneIntersection(vec3(0.,0.,0.), iDir.xyz, 1., vec3(0.,-1.,0.)));
			stepY = 1;
		}else{
			tMaxY = length(rayPlaneIntersection(origGridSpace, iDir.xyz, -float(Y), vec3(0.,1.,0.))-origGridSpace);
			tDeltaY = length(rayPlaneIntersection(vec3(0.,0.,0.), iDir.xyz, -1., vec3(0.,1.,0.)));
			stepY = -1;
		}

		if(iDir.z > 0.){
			tMaxZ = length(rayPlaneIntersection(origGridSpace, iDir.xyz, float(Z)+1., vec3(0.,0.,-1))-origGridSpace);
			tDeltaZ = length(rayPlaneIntersection(vec3(0.,0.,0.), iDir.xyz, 1., vec3(0.,0.,-1.)));
			stepZ = 1;
		}else{
			tMaxZ = length(rayPlaneIntersection(origGridSpace, iDir.xyz, -float(Z), vec3(0.,0.,1.))-origGridSpace);
			tDeltaZ = length(rayPlaneIntersection(vec3(0.,0.,0.), iDir.xyz, -1., vec3(0.,0.,1.)));
			stepZ = -1;
		}
		...
		{	
			...
			while (startPos != 65535.){
				...
				if (spheresIndex.r != 65535.){
					...
					intersectSphere(sphereInfo.xyz, sphereInfo.w, iPoint, iDir.xyz, point, normal);
				}
		...
		if(point.w != 100000000000.){ // intersect
		
			vec3 outgoingDirection = reflect(iDir.xyz, normal.xyz);
			
			gl_FragData[0] = vec4(outgoingDirection.xyz,0.);// store uv coordinates + heigh
			gl_FragData[1] = vec4(point.xyz + normal.xyz*0.001,100000000000.);// store reflected direction + status
			
		
		}else{ // does not intersect
		
			gl_FragData[0] = vec4(iDir.xyz,1.); // check if it touch boundaries, if not, valid output!!!

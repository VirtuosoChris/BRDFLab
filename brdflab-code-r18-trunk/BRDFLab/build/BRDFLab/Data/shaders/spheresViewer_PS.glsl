...
void main()																					
{
	...
	vec3 iPoint = gl_TexCoord[0].xyz; // point
	vec3 iDir = normalize(gl_TexCoord[0].xyz-eyePos.xyz);//normalize(gl_TexCoord[1].xyz);
	...
	if(point.w != 100000000000.){
		...
		vec3 u = reflect(iDir.xyz,normal);
		vec3 v = normalize(lightPos.xyz-point.xyz);
